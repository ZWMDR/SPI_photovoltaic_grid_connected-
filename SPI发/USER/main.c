...
int main(void)
{
	...
	u16 battery_Voltage;
	u16 battery_Current;
	//u8 frequency_change_flag;//频率变化标志
	u8 target_change_flag;//预设电压目标改变标志
	u16 TARGET;//设定目标
	u16 voltage_F;//ADC采样值
	u8 exception_flag;//异常状态标志
	u8 lst_exception_flag;//异常状态维持标志
	//s16 frequency_error;//频率误差
	//s16 phase_error;//相位误差
	///s16 voltage_error;//电压误差
	u16 steady_count;//稳定状态计数
	//u16 shield_count;//屏蔽检测状态计数
	PID vcc_PID,phase_PID,frequency_PID,battery_PID;//PID结构
	float delta_rate;//占空比改变值
	float delta_phase;
	u16 PID_step;//PID调控稳态
	...
	//ADC采样初始化（连续转换只能使用ADC1）
	ADC_cs.channel_num=4;//通道数
			...
			else if(working_mode==1)//MPPT模式
			{
				if(ADC_flag)
				{
					...
					*/
					...
					/*
					else if(battery_voltage<25)//欠压保护
					{
						LED1=0;
						relay_module_Close(&relay_module);//关断输出
						relay_module_Open(&buzzer);//蜂鸣器警报
					...
					*/
					...
					SPI_send_buff[3]=voltage_F;//输入电压
					SPI_send_buff[4]=Current;//输出功率
					ADC_flag=0;
					lst_TARGET=TARGET;
				}
			}
