...
int main(void)
{
	...
	while(1)
	{
		if(capture_flag)
		{
			if(DMA_SPI_buff_TX[1]<40000 && DMA_SPI_buff_TX[1]>10000)
			{
				TIM6->ARR=(u16)(DMA_SPI_buff_TX[1]*0.18)-1;
			}
			capture_flag=0;
		}
		...
		{
			//LED1=~LED1;
			get_average_duplex(DMA_buff,DMA_buff_len_ADC,&voltage_REF,&voltage_F);
			//printf("%d\r\n",voltage_REF);
			SPI_send_buff[5]=voltage_REF;
			SPI_send_buff[6]=voltage_F;
			//voltage_scale_rate+=get_iIncpid(&vcc_PID,voltage_F,target*0.66094546)*0.00024414;
			voltage_scale_rate+=get_iIncpid(&vcc_PID,voltage_REF,1200*0.767689697)*0.00024414;
